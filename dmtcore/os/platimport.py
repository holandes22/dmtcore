...
def platimport(parent_module, property_name, submodule = None):
    ...
    try:
        ...
        # TODO: Maybe chain the error, we can enter here if a the platimport
        # worked but some submodule raised an import error
        module_logger.error("Import error of module {0}".format(platform_module))
        raise NotImplementedError("Error while importing. Is {0} supported/implemented?".format(os_name))
    except AttributeError:
        module_logger.error("Error getting property {0} from module {1}".format(property_name, platform_module))
